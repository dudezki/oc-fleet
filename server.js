...
async function collectState() {
  ...
  // Sessions
  let sessions = { active_count: 0, recent: [] };
  try {
    const response = await fetch(`http://127.0.0.1:${PROXY_PORT}/fleet-api/session/list`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ org_id: ORG_ID, limit: 10 }),
      signal: AbortSignal.timeout(4000),
    });
    const d = await response.json();
    const rows = d.sessions || [];
    sessions = {
      active_count: rows.filter(s => !s.ended_at).length,
      recent: rows,
    };
  } catch {}

  return { instances, proxy, memories, handoffs, bindings, accounts, conversations, tasks, skills, sessions, updatedAt: new Date().toISOString() };
}
...
// GET /api/sessions?agent_id=&platform_chat_id=&limit=
app.get('/api/sessions', async (req, res) => {
  const { agent_id, platform_chat_id, limit } = req.query;
  try {
    const body = { org_id: ORG_ID };
    if (agent_id) body.agent_id = agent_id;
    if (platform_chat_id) body.platform_chat_id = platform_chat_id;
...
// GET /api/sessions/active?agent_id=&platform_chat_id=
app.get('/api/sessions/active', async (req, res) => {
  const { agent_id, platform_chat_id } = req.query;
  try {
    const body = { org_id: ORG_ID };
    if (agent_id) body.agent_id = agent_id;
    if (platform_chat_id) body.platform_chat_id = platform_chat_id;
    const response = await fetch(`http://127.0.0.1:${PROXY_PORT}/fleet-api/session/active`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(body),
      signal: AbortSignal.timeout(8000),
...
app.post('/api/sessions/reset', async (req, res) => {
  try {
    const body = { org_id: ORG_ID, ...req.body };
    const response = await fetch(`http://127.0.0.1:${PROXY_PORT}/fleet-api/session/reset`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(body),
      signal: AbortSignal.timeout(8000),
...
app.post('/api/sessions/resume', async (req, res) => {
  try {
    const body = { org_id: ORG_ID, ...req.body };
    const response = await fetch(`http://127.0.0.1:${PROXY_PORT}/fleet-api/session/resume`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(body),
      signal: AbortSignal.timeout(8000),
...
app.get('/api/sessions/:id/messages', async (req, res) => {
  ...
  try {
    const response = await fetch(`http://127.0.0.1:${PROXY_PORT}/fleet-api/conversation/history`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ session_id: id, limit, org_id: ORG_ID }),
      signal: AbortSignal.timeout(8000),
    });
    const data = await response.json();
    res.json(data);
...
app.get('/api/sessions/stats', async (req, res) => {
  try {
    ...
    const today = sessions.filter(s => s.started_at && s.started_at >= todayStart).length;
    const totalMsgs = sessions.reduce((a, s) => a + (s.message_count || 0), 0);
    const avg_messages = total ? Math.round(totalMsgs / total) : 0;
    const recent = [...sessions].sort((a, b) => new Date(b.started_at || 0) - new Date(a.started_at || 0)).slice(0, 5);
    res.json({ total, active, today, avg_messages, recent });
  } catch (err) {
    res.status(502).json({ error: 'Proxy unreachable', detail: err.message });
  }
});

// GET /api/sessions/reports — analytics data (last 30 days, by agent, dm vs gc)
app.get('/api/sessions/reports', async (req, res) => {
  try {
    const response = await fetch(`http://127.0.0.1:${PROXY_PORT}/fleet-api/session/list`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    const by_day = Object.entries(byDay).map(([date, count]) => ({ date, count }));
    const agentMap = {};
    for (const s of sessions) {
      const key = s.agent_name || (s.agent_id ? s.agent_id.slice(0, 8) : 'Unknown');
      if (!agentMap[key]) agentMap[key] = { agent: key, total: 0, active: 0, total_messages: 0 };
      agentMap[key].total++;
      if (!s.ended_at) agentMap[key].active++;
      agentMap[key].total_messages += (s.message_count || 0);
    }
    const by_agent = Object.values(agentMap)
      .map(a => ({ ...a, avg_messages: a.total ? Math.round(a.total_messages / a.total) : 0 }))
      .sort((a, b) => b.total - a.total);
    const dm_count = sessions.filter(s => !s.is_group && s.chat_type !== 'group').length;
    const gc_count = sessions.filter(s => s.is_group || s.chat_type === 'group').length;
    const reset_count = sessions.filter(s => s.reset_by).length;
    res.json({ by_day, by_agent, dm_count, gc_count, reset_count, total: sessions.length });
  } catch (err) {
    res.status(502).json({ error: 'Proxy unreachable', detail: err.message });
  }
});
...
app.get('/api/agents/:slug/sessions', async (req, res) => {
  ...
  try {
    const ar = await pgPool.query('SELECT id FROM fleet.agents WHERE slug=$1 AND org_id=$2', [slug, ORG_ID]);
    if (!ar.rows.length) return res.status(404).json({ error: 'Agent not found' });
    const agent_id = ar.rows[0].id;
    const response = await fetch(`http://127.0.0.1:${PROXY_PORT}/fleet-api/session/list`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ org_id: ORG_ID, agent_id, limit }),
      signal: AbortSignal.timeout(8000),
    });
    const data = await response.json();
    const sessions = Array.isArray(data) ? data : (data.sessions || []);
    const total = sessions.length;
    const active = sessions.filter(s => !s.ended_at).length;
    res.json({ sessions, total, active });
  } catch (err) {
    res.status(502).json({ error: 'Proxy unreachable', detail: err.message });
  }
});
